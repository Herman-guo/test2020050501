...
public class OrderDaoImple implements OrderDao {
	...
	public void saveOrder(Connection conn, Order order) {
		...
		String sql = "insert into t_orders_estore values (?,?,?,?,?,?)";
	...
	public void saveOrderItem(Connection conn, OrderItem orderItem) {
		QueryRunner runner = new QueryRunner();
		String sql = "insert into t_orderitem_estore values (?,?,?,?,?)";
	...
	public List<Order> findByUid(String uid) {
		...
		String sql = "select * from t_orders_estore where uid = ?";
		// list集合中包含了1号订单和2号订单
		List<Order> list;
		try {
			...
			for (Order order : list) {
				...
				// 多表的查询，查询的是订单项和书的信息
				String sql2 = "select * from t_orderitem_estore o ,t_book_estore b where o.bid = b.bid and o.oid = ?";
	...
	public Order findByOid(String oid) {
		// 查询当前的订单
		QueryRunner runner = new QueryRunner(MyJdbcUtil.getDataSource());
		String sql = "select * from t_orders_estore where oid = ?";
		Order order;
		try {
	...
	public void updateOrder(Order order) {
		QueryRunner runner = new QueryRunner(MyJdbcUtil.getDataSource());
		String sql = "update t_orders_estore set address = ? ,state = ? where oid = ?";
		try {
	...
	public List<Order> findAll() {
		// 先来查询订单的信息
		QueryRunner runner = new QueryRunner(MyJdbcUtil.getDataSource());
		String sql = "select * from t_orders_estore";
		// list集合中包含了1号订单和2号订单
		List<Order> list;
		try {
	...
	public List<Order> findByState(int state) {
		// 先来查询订单的信息
		QueryRunner runner = new QueryRunner(MyJdbcUtil.getDataSource());
		String sql = "select * from t_orders_estore where state = ?";
		// list集合中包含了1号订单和2号订单
		List<Order> list;
		try {
	...
	public void updateByState(String oid,int state){
		QueryRunner runner = new QueryRunner(MyJdbcUtil.getDataSource());
		String sql  = "update t_orders_estore set state = ? where oid = ?";
		try {
