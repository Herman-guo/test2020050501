...
public class BookDaoImple implements BookDao {
	...
	public List<Book> findAll() {
		...
		try {
			return runner.query("select * from t_book_estore where isdel = ?", new BeanListHandler<Book>(Book.class), 0);
	...
	public List<Book> findByCid(String cid) {
		QueryRunner runner = new QueryRunner(MyJdbcUtil.getDataSource());
		try {
			return runner.query("select * from t_book_estore where cid = ? and isdel = ?", new BeanListHandler<Book>(Book.class), cid,0);
		} catch (SQLException e) {
			e.printStackTrace();
			throw new RuntimeException(e);
		}
	...
	public Book findByBid(String bid) {
		QueryRunner runner = new QueryRunner(MyJdbcUtil.getDataSource());
		try {
			return runner.query("select * from t_book_estore where bid = ?", new BeanHandler<Book>(Book.class), bid);
		} catch (SQLException e) {
			e.printStackTrace();
			throw new RuntimeException(e);
		}
	...
	public void updateByCid(String cid) {
		QueryRunner runner = new QueryRunner(MyJdbcUtil.getDataSource());
		String sql = "update t_book_estore set cid = null where cid = ? and isdel = 1";
		try {
	...
	public PageBean<Book> findByPage(int pageCode, int pageSize) {
		...
		QueryRunner runner = new QueryRunner(MyJdbcUtil.getDataSource());
		try {
			String countSql = "select count(*) from t_book_estore where isdel = 0";
			// 查询数据库，查询数据库中图书的总数量（isdel=0）
			...
			// 查询当前页显示的图书的信息
			String limitSql = "select * from t_book_estore where isdel = 0 limit ?,?";
	...
	public void save(Book book) {
		QueryRunner runner = new QueryRunner(MyJdbcUtil.getDataSource());
		String sql = "insert into t_book_estore values (?,?,?,?,?,?,?)";
	...
	public void updateBook(Book book) {
		QueryRunner runner = new QueryRunner(MyJdbcUtil.getDataSource());
		String sql = "update t_book_estore set bname = ? , price = ? ,author = ? ,image = ? ,cid = ?, isdel = ? where bid = ?";
	...
	public void deleteByBid(String bid) {
		QueryRunner runner = new QueryRunner(MyJdbcUtil.getDataSource());
		String sql = "update t_book_estore set isdel = 1 where bid = ?";
		try {
